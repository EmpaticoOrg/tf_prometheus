...
class { '::prometheus':
  ...
  scrape_configs => [ { 'job_name'=> 'prometheus', 'scrape_interval'=> '1s', 'scrape_timeout'=> '1s', 'target_groups'=> [ { 'targets'=> [ 'localhost:9090' ], 'labels'=> { 'alias'=> 'Prometheus'} } ] }
                      { 'job_name'=> 'host', 'ec2_sd_configs'=> { 'region'=> 'us-east=1', 'port'=> 9100 } } ]
}
EOF
